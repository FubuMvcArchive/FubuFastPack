...
{
    public class NoNestedTransactionProcessor : ITransactionProcessor
    {
        ...
        public NoNestedTransactionProcessor(IContainer container)
        {
        ...
        private void execute(Action<IContainer> action)
        {
            ...
            if (!container.Model.HasDefaultImplementationFor<ITransactionBoundary>())
            {
                throw new ApplicationException(
                    "This container has already been disposed or does not have an ITransactionBoundary registration");
            }

            var boundary = container.GetInstance<ITransactionBoundary>();
            boundary.Start();
            action(container);
            boundary.Commit();
